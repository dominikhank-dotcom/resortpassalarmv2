...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const handleDatePresetChange = (preset: string) => {
    ...
    let start = new Date();
    const now = new Date();
    if (preset === 'today') { start = new Date(); end = new Date(); }
    else if (preset === 'yesterday') { const d = new Date(); d.setDate(d.getDate() - 1); start = new Date(d); end = new Date(d); }
    else if (preset === 'last7') { start.setDate(end.getDate() - 7); }
    else if (preset === 'last28') { start.setDate(end.getDate() - 28); }
  ...
  const handleSavePartnerSettings = async () => {
      try {
          ...
          alert("Partner Provision gespeichert!");
      } catch (e: any) { alert(e.message); }
  };
  ...
  const handleSaveUrls = async () => {
      try { await updateSystemSettings('url_gold', productUrls.gold); await updateSystemSettings('url_silver', productUrls.silver); alert("URLs gespeichert!"); } catch(e:any) { alert(e.message); }
  }
      ...
      return (
          <div className="flex justify-between items-center mt-4 p-2 bg-slate-50 rounded-lg">
             <div className="flex items-center gap-2">
                 ...
                 <select value={itemsPerPage} onChange={e => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="bg-white border border-slate-300 rounded px-2 py-1 text-sm outline-none">
             ...
             <div className="flex gap-2">
                 <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => Math.max(1, p - 1))} className="p-1 rounded border bg-white disabled:opacity-50 hover:bg-slate-100"><ChevronLeft size={16} /></button>
                 <span className="text-sm flex items-center px-2">Seite {currentPage} von {totalPages}</span>
                 <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} className="p-1 rounded border bg-white disabled:opacity-50 hover:bg-slate-100"><ChevronRight size={16} /></button>
             </div>
          </div>
      );
  };
             ...
             <div className="flex gap-1 bg-slate-800 p-1 rounded-lg overflow-x-auto max-w-full no-scrollbar">
                 {[
                   ...
                   { id: 'settings', label: 'Einstellungen', icon: Sliders }
                     ...
                     <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-white hover:bg-slate-700'}`}>
                        <tab.icon size={16} />
                        {tab.label}
                     </button>
                 ))}
              ...
              <div className="space-y-6 animate-in fade-in">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                      ...
                      <div className="bg-white p-4 rounded-xl shadow-sm border border-red-200 bg-red-50/50">
                          <div className="flex justify-between items-start">
                              <div><p className="text-red-800 text-xs uppercase font-bold">Neue Kündigungen</p><p className="text-2xl font-bold text-slate-900 mt-1">{stats.newCancellations}</p></div>
                              <XCircle className="text-red-600 opacity-50" size={20} />
                          </div>
                      </div>
                      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                          <div className="flex justify-between items-start">
                              <div><p className="text-slate-500 text-xs uppercase font-bold">Umsatz (MRR)</p><p className="text-2xl font-bold text-green-600 mt-1">{stats.revenue.toFixed(2)} €</p></div>
                      ...
                      <div className="bg-white p-4 rounded-xl shadow-sm border border-indigo-100 bg-indigo-50/50">
                          <div className="flex justify-between items-start">
                              <div><p className="text-indigo-800 text-xs uppercase font-bold">Neue Kunden</p><p className="text-2xl font-bold text-indigo-700 mt-1">+{stats.newCustomers}</p></div>
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {visiblePartners.map((p: any) => (
                                    <tr key={p.id}>
                                        <td className="px-4 py-3"><div className="font-bold text-slate-900">{p.name}</div><div className="text-xs text-slate-500">{p.email}</div></td>
                                        <td className="px-4 py-3"><span className="font-mono bg-blue-50 text-blue-700 px-2 py-1 rounded font-bold">{p.ref_code || 'pending'}</span></td>
                                        <td className="px-4 py-3 text-slate-500">{p.website || '-'}</td>
                                        <td className="px-4 py-3 font-bold text-slate-900">{p.total_commission ? p.total_commission.toFixed(2) : '0.00'} €</td>
                                        <td className="px-4 py-3">{p.referred_count > 0 ? <span className="text-green-600 text-xs font-bold uppercase bg-green-100 px-2 py-1 rounded">Aktiv ({p.referred_count})</span> : <span className="text-slate-400 text-xs uppercase bg-slate-100 px-2 py-1 rounded">Keine Refs</span>}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-140px)] animate-in fade-in">
                 <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                     <div className="p-4 border-b border-slate-100 bg-slate-50 font-bold text-slate-700 flex justify-between items-center">
                        <span>Templates</span>
                        <Button size="sm" variant="secondary" onClick={loadTemplates} disabled={isTemplatesLoading}><RefreshCw size={14} className={isTemplatesLoading ? "animate-spin" : ""} /></Button>
                     </div>
                     <div className="overflow-y-auto flex-1 p-2 space-y-2">
                         {templates.map(t => (
                             <div key={t.id} onClick={() => setSelectedTemplate(t)} className={`p-3 rounded-lg cursor-pointer border transition-all ${selectedTemplate?.id === t.id ? 'bg-blue-50 border-blue-200 shadow-sm' : 'border-transparent hover:bg-slate-50'}`}>
                                 <div className="flex justify-between items-start"><div className="font-bold text-slate-900 text-sm">{t.name}</div>{t.isEnabled === false && <div className="bg-red-100 text-red-600 text-[10px] px-1 rounded font-bold uppercase">Disabled</div>}</div>
                                 <div className="text-xs text-slate-500 truncate">{t.subject || 'SMS'}</div>
                             </div>
                         ))}
                     </div>
                 </div>
                     ...
                     {selectedTemplate ? (
                         <>
                             <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                                 <div><h3 className="font-bold text-slate-900">{selectedTemplate.name}</h3><p className="text-xs text-slate-500">{selectedTemplate.description}</p></div>
                                 <div className="flex items-center gap-2"><Button size="sm" onClick={handleSaveTemplate}><Save size={14} className="mr-2"/> Speichern</Button></div>
                             </div>
                             <div className="p-6 flex-1 overflow-y-auto space-y-4">
                                 {!selectedTemplate.id.includes('sms') && (<div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Betreff</label><input type="text" value={selectedTemplate.subject} onChange={e => setSelectedTemplate({...selectedTemplate, subject: e.target.value})} className="w-full p-2 border rounded font-medium" /></div>)}
                                 <div className="flex-1 flex flex-col h-full"><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Inhalt</label><textarea value={selectedTemplate.body} onChange={e => setSelectedTemplate({...selectedTemplate, body: e.target.value})} className="w-full flex-1 p-2 border rounded font-mono text-sm min-h-[300px]" /></div>
                             </div>
                         </>
                     ) : (<div className="flex-1 flex flex-col items-center justify-center text-slate-400"><Mail size={48} className="mb-4 opacity-20"/><p>Wähle ein Template aus der Liste.</p></div>)}
                 </div>
             </div>
          )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
              <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                  <div className="p-6 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white z-10">
                      <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2"><Users className="text-blue-600" /> Kundendetails</h2>
                  ...
                  <div className="p-6 space-y-6">
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                          ...
                          <div className="flex justify-between items-center">
                              <div>
                                  <p className="text-lg font-bold">{selectedCustomer.subscription?.status === 'active' ? (selectedCustomer.subscription?.cancel_at_period_end ? <span className="text-amber-600">Gekündigt (läuft aus)</span> : <span className="text-green-600">Aktiv</span>) : <span className="text-slate-500">Inaktiv / Gekündigt</span>}</p>
