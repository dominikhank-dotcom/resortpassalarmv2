...
export const UserDashboard: React.FC<UserDashboardProps> = ({ navigate, productUrls, prices }) => {
    ...
    if (user) {
        ...
        if (sub) {
            ...
            setSubscriptionDetails({
                ...
                isCanceled: sub.cancel_at_period_end === true // READ CANCEL STATUS
            });
        } else {
  ...
  const handleTestAlarm = async () => { const activeMethods = []; if (notifications.emailEnabled) activeMethods.push("E-Mail"); if (notifications.smsEnabled) activeMethods.push("SMS"); if (activeMethods.length === 0) { alert("Bitte Benachrichtigungsmethode aktivieren."); return; } setIsSendingAlarm(true); try { await sendTestAlarm(notifications.email, notifications.sms, notifications.emailEnabled, notifications.smsEnabled); const newEntry: LogEntry = { id: Math.random().toString(36).substr(2, 9), date: new Date().toLocaleString(), type: notifications.emailEnabled ? 'EMAIL' : 'SMS', message: 'Test-Alarm erfolgreich gesendet' }; setAlarmHistory(prev => [newEntry, ...prev]); alert("Test verschickt!"); } catch (error) { alert("Fehler: " + error); } finally { setIsSendingAlarm(false); } };
                ...
                <div className="flex-1">
                  {subscriptionStatus === 'FREE' ? (
                    ...
                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 mb-4">
                        ...
                        {/* CANCELED OR ACTIVE STATUS TEXT */}
